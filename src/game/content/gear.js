...
function touchGearSpriteCacheKey(key) {
    ...
    while (gearSpriteCacheOrder.length > GEAR_SPRITE_CACHE_MAX) {
        ...
        if (typeof Engine !== 'undefined' && Engine.Graphics && Engine.Graphics.SpriteCache) {
            Engine.Graphics.SpriteCache.release(evict);
        }
    }
}
...
function getCachedGearSprite(gear) {
    const key = `gear:${buildGearSpriteCacheKey(gear)}`;
    if (typeof Engine !== 'undefined' && Engine.Graphics && Engine.Graphics.SpriteCache) {
        const sprite = Engine.Graphics.SpriteCache.get(key, () => bakeGearSprite(gear));
