...
const STAR_CONFIG = {
    ...
    shootRange: 500,               // Ideal shooting distance (pixels) - well outside player light (400px)
...
class StarEnemy extends EnemyBase {
    constructor(x, y) {
        ...
        // Stealth AI adjustments
        if (this.stealthEnemy) {
            // Stealth variants stay further back to utilize darkness
            this.shootRange = 650;
            this.minRange = 600;
            this.maxRange = 700;
        }

        // Relocation behavior (move after shooting)
        this.relocateTimer = 0;
        this.relocateDirection = { x: 0, y: 0 };
        this.relocateDuration = 1.5; // How long to move after shooting
    }
    ...
    update(deltaTime) {
        ...
        // Update relocation timer
        if (this.relocateTimer > 0) {
            this.relocateTimer -= deltaTime;
        }

        // Distance-based AI with strafing and separation
        // Override with relocation behavior if active
        if (this.relocateTimer > 0 && this.stealthEnemy) {
            // Force movement in relocation direction
            let moveX = this.relocateDirection.x;
            let moveY = this.relocateDirection.y;

            // Still apply separation to avoid clumping
            ...
            if (sepDist > 0) {
                ...
                moveX = moveX * 0.7 + sepNormX * 0.3 * sepStrength;
                moveY = moveY * 0.7 + sepNormY * 0.3 * sepStrength;
            ...
            // Move fast during relocation
            this.applySmoothedDirectionalMovement(moveX, moveY, this.moveSpeed * 1.2, deltaTime, 0.2);
    ...
    shoot(targetX, targetY, isBurstFire = false, skipTelegraph = false, options = {}) {
        ...
        // Stealth behavior: Relocate after shooting to avoid triangulation
        if (this.stealthEnemy) {
            this.relocateTimer = this.relocateDuration;

            // Pick a direction roughly perpendicular to the target to strafe away
            // Or just a random direction that isn't directly towards/away from player
            const angleToTarget = Math.atan2(dy, dx);
            const moveAngle = angleToTarget + (Math.random() < 0.5 ? Math.PI / 2 : -Math.PI / 2) + (Math.random() * 0.5 - 0.25);

            this.relocateDirection = {
