...
(function () {
    ...
    function getProcGrid(layout) {
        if (!layout || !layout.grid) return null;
        if (typeof Engine === 'undefined' || !Engine.Proc || !Engine.Proc.Grid) return null;
        if (layout._procGrid && layout._procGridSource === layout.grid) return layout._procGrid;
        const grid = new Engine.Proc.Grid(layout.cols, layout.rows, {
            data: layout.grid,
            cellSize: layout.cellSize || DEFAULT_CELL_SIZE,
            outside: (Engine.Proc.Cell && Engine.Proc.Cell.BLOCKED != null)
                ? Engine.Proc.Cell.BLOCKED
                : BLOCKED
        });
        Object.defineProperty(layout, '_procGrid', {
            value: grid,
            writable: true,
            configurable: true,
            enumerable: false
        });
        Object.defineProperty(layout, '_procGridSource', {
            value: layout.grid,
            writable: true,
            configurable: true,
            enumerable: false
    ...
    function getRadiusProcGrid(layout, entityRadius) {
        const base = getProcGrid(layout);
        if (!base) return null;
        const radiusKey = Math.max(0, Math.ceil(entityRadius || 0));
        if (!layout._radiusProcGrids) {
            Object.defineProperty(layout, '_radiusProcGrids', {
                value: new Map(),
                writable: true,
                configurable: true,
                enumerable: false
        ...
        if (layout._radiusProcGrids.has(radiusKey)) {
            return layout._radiusProcGrids.get(radiusKey);
        }
        const grid = base.clone();
        if (radiusKey > 0) {
            for (let row = 0; row < layout.rows; row++) {
                for (let col = 0; col < layout.cols; col++) {
                    if (grid.get(col, row) === BLOCKED) continue;
                    const center = cellCenter(layout, col, row);
                    if (!isPointWalkable(layout, center.x, center.y, entityRadius)) {
                        grid.set(col, row, BLOCKED);
                    }
                }
            }
        }
        layout._radiusProcGrids.set(radiusKey, grid);
        return grid;
    }
    ...
    function hasPathBetween(layout, from, to) {
        ...
        if (!procGrid) return false;
        const start = worldToCell(layout, from.x, from.y);
        const goal = worldToCell(layout, to.x, to.y);
        const path = procGrid.findPath(
            { x: start.col, y: start.row },
            { x: goal.col, y: goal.row }
        );
    ...
    function findPath(layout, from, to, radius, options) {
        ...
        if (radiusGrid) {
            const cellPath = radiusGrid.findPath(
                { x: start.col, y: start.row },
                { x: goal.col, y: goal.row },
                { diagonal: true }
            );
            if (cellPath.length) {
                ...
                for (let i = 0; i < cellPath.length; i++) {
                    ...
                    if (i > 0) {
                        ...
                        if (!canTraverseBetweenCells(layout, prev.x, prev.y, cell.x, cell.y, entityRadius)) {
                            pathValid = false;
                            break;
                        }
                        const dc = cell.x - prev.x;
                        const dr = cell.y - prev.y;
                        if (dc !== 0 && dr !== 0) {
                            const centerA = cellCenter(layout, prev.x + dc, prev.y);
                            const centerB = cellCenter(layout, prev.x, prev.y + dr);
                            if (!isPointWalkable(layout, centerA.x, centerA.y, entityRadius)
                                || !isPointWalkable(layout, centerB.x, centerB.y, entityRadius)) {
                                pathValid = false;
                                break;
                            }
                        }
                    ...
                    path.push({ x: center.x, y: center.y, col: cell.x, row: cell.y });
                ...
                if (pathValid
                    && !(path.length >= maxPathLength
                        && (path[path.length - 1].col !== goal.col || path[path.length - 1].row !== goal.row))) {
    ...
    function resolveCircleCollision(layout, x, y, radius, previousX, previousY) {
        ...
        const procGrid = getProcGrid(layout);
        if (procGrid) {
            const resolved = procGrid.resolveCircle({ x, y, radius }, { iterations: 4 });
            if (isPointWalkable(layout, resolved.x, resolved.y, radius)) {
                return { x: resolved.x, y: resolved.y, collided: true };
            }
        }
