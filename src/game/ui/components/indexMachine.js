(function () {
	...
	function toggle(show) {
		...
		if (show) {
			if (!indexModalEntry) {
				indexModalEntry = GameUI.openModal(layer, {
					display: 'block',
					closeOnEscape: true,
					onClose: () => {
						indexModalEntry = null;
						open = false;
						if (typeof Game !== 'undefined') Game.showingIndexMachine = false;
						stopPreviewAnimation();
					}
				});
			}
		...
		} else if (indexModalEntry) {
			GameUI.closeModal(indexModalEntry);
			indexModalEntry = null;
			stopPreviewAnimation();
		} else {
			stopPreviewAnimation();
